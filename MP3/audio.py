...
def read_file(filename):
	...
	while n < 3665: #this is a constant found by printing out n (number of lines) and finding where we go out of bounds
		for idx, i in enumerate(range(n, n + 25)): #this is the loop that we can do our work in
			...
			for col, char in enumerate(lines[i]):
				...
				if char == '%': #low energy
					#print char
					lowprev = charlist[idx * 10 + col][0] # need to save the previous values since you can't edit tuple values in python
					highprev = charlist[idx * 10 + col][1]
					charlist[idx * 10 + col] = (lowprev + 1, highprev) # increase the count
				else: 
					#print char
...
def test(filename, k, outputfile, likelihood_no, likelihood_yes):
	f = open(filename)
	...
	while n < 1397:
		...
		for idx, i in enumerate(range(n, n + 25)): #this is the loop that we can do our work in
			for col, char in enumerate(lines[i]):
				...
				if char == ' ': #high energy; keep a tuple of (P(W(i,j)| "NO"), P(W(i, j)|"YES"))
		...
		# calculate the values for MAP decision
		...
		if label_no > label_yes: 
			write_file(outputfile, "NO\n")
		else:
			write_file(outputfile, "YES\n")
		#write_file('temp1.txt', testlines) - WORKING
		#del testlines[:] - WORKING

		#clear charlist so we can use it again
		del charlist[:]
		charlist = [(0, 0)] * 250
...
def main(samplename): 
	likelihood_no = train(0.1)[0]
	likelihood_yes = train(0.1)[1]
	test("no_test.txt", 0.1, "no_results.txt", likelihood_no, likelihood_yes)
	test("yes_test.txt", 0.1, "yes_results.txt", likelihood_no, likelihood_yes)
